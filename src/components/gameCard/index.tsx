...
export function GameCard({ id, title, date, publisher, genre, img}: GameCardProps) {
    ...
    return (
        <Grid item xs={12} sm={6} md={4} lg={3} xl={3}>
            <Card sx={{height: '100%', boxShadow: '2px 3px 9px 0px #8DFD1B', backgroundColor: '#1A1A1A', color: '#FFFFFF', cursor: 'pointer', border: '1px solid #8DFD1B', paddingTop: '24px'}} onClick={() => navigate(`/game/${id}`)}>
                <CardMedia
                ...
                <CardContent>
                    ...
                    <Typography gutterBottom variant="body2" display="flex" alignItems="center" color="text.secondary" sx={{color: '#FFFFFF'}}>
                        ...
                        <Typography variant="body2" display="flex" alignItems="center" color="text.secondary" sx={{color: '#FFFFFF'}}>
                        <ExtensionIcon sx={{marginRight: "5px" }}></ExtensionIcon>
                        {genre}
                    </Typography>
                </CardContent>
            </Card>
